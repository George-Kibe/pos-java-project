...
> `purchasing-service` and `sales-service` are built, tested and running under Docker Compose.
> Phase 10's `payment-service` is built and running - card and cash settle end to end, M-Pesa is
> verified against a fake of Daraja - and waits only on a real sandbox STK Push, which needs the
> Daraja credentials in `.env`. 639 tests in the build. The frontend starts at phase 13.
> Code is built phase by phase per
